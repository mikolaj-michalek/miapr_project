...
ROBOT_NAME = "robot"          
CSV_FILENAME = "benchmark_averages_parking_garage.csv" # Zmieniona nazwa pliku
...
class MBFBenchmark(Node):
    ...
    def reset_robot(self):
        """Teleport robota na pozycję startową"""
        ...
        try:
            ...
            time.sleep(1.0) # Czas na stabilizację robota
        except Exception as e:
            self.get_logger().error(f"Nie udało się zresetować robota: {e}")

    def calculate_path_curvature(self, path_poses):
        """Obliczanie skumulowanej krzywizny ścieżki (suma kątów skrętów) w stopniach."""
        if len(path_poses) < 3:
            return 0.0
        ...
        for i in range(1, len(path_poses) - 1):
            ...
            # Zabezpieczenie przed dzieleniem przez zero przy bardzo bliskich punktach
    ...
    def run_benchmark(self):
        ...
        self.get_logger().info("Rozpoczęcie testów wydajnosci (Tylko Planowanie).")

        for planner in planners:
            self.reset_robot() # Resetujemy raz przed serią testów danego algorytmu
            self.get_logger().info(f"\n==================================================")
            self.get_logger().info(f"Testowanie planera globalnego: {planner} ({NUM_RUNS} iteracji)")
            ...
            for i in range(NUM_RUNS):
                ...
                if path_result.outcome != 0:
                    self.get_logger().error(f"[{i+1}/{NUM_RUNS}] Blad planowania")
                    continue
                ...
                self.get_logger().info(f"[{i+1}/{NUM_RUNS}] Czas: {planning_time:.2f}ms | Dlugosc: {path_length:.2f}m | Pkt: {waypoints_count}")
        ...
        self.get_logger().info(f"\nTesty zakończone. Dane zapisano w pliku: {CSV_FILENAME}")
